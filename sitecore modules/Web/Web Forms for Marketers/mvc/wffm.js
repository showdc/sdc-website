  ...
  $scw.widget("wffm.wffmForm", {
    _create: function () {
	  ...
	  if (isMobileScreen()) {
                ...
                /*$('select:not(.original)').on('shown.bs.select',functions(){
                ...
                $('select:not(.original)').selectpicker({
                 ...
                 $('.bootstrap-select .btn').attr('title','');
                /*$('select:not(.original)').on('loaded.bs.select',function(){
                    $(this).parent().find('.btn').attr('title','');                  
                }) ;
                $('select:not(.original)').on('rendered.bs.select',function(){
                    $(this).parent().find('.btn').attr('title','');
      ...
      if (ajaxForm) {
          $scw(form).find("input[type=submit]").hover(function () {
              $(this).css("background-color", "transparent")
          });

        $scw(form).submit(function (e) {
          if ($scw(this).valid()) {
			if ($(form).children().last().children().last().attr("name") == "doneMessage")
			{
				$(form).children().last().children().last().remove();
			}
            var headersList = {
			...
			$scw(form).find("input[type=submit]").prop('disabled', true);
			$scw(form).find("input[type=submit]").css({ 'cursor': 'wait' });
              ...
              $scw.ajax({
                ...
                success: function (res) {
				  ...
				  $(document.body).css({'cursor' : 'default'});
					$scw(form).find("input[type=submit]").prop('disabled', false);
					$scw(form).find("input[type=submit]").css({'cursor' : 'default'});
                },
    ...
    formSubmitSuccess: function(form, res) {
      ...
      if (res.indexOf("<script") === 0) {
		...
		var resultHtml = $.parseHTML(res);
		var backResult = null;
		$(resultHtml).each(function (index, e){
		...
		if (backResult != null)
		{
		    $form.html("<form><div class='form-submit-border'><p class='message' name='doneMessage'>"
		        + $(backResult).text() + "</p></div></form");
		    $(window).scrollTop($("#contact-menu-control").offset().top - $("#header").height());
		}
      }
